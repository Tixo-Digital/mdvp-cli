...
describe('public adoption surface', () => {
  ...
  it('documents an ethical adoption loop with measurable conversion surfaces', () => {
    assert.match(adoption, /## Ethical Star Growth Loop/)
    assert.match(adoption, /Ask for critique of the signal, docs, or integration path instead of asking for stars/)
    assert.match(adoption, /reject fake stars, star swaps, spam comments, mass DMs, review manipulation/)
    assert.match(adoption, /## Audiences And Channels/)
    assert.match(adoption, /AI frontend builders/)
    assert.match(adoption, /CI and release engineers/)
    assert.match(adoption, /Design-system teams/)
    assert.match(adoption, /MCP and agent tool users/)
    assert.match(adoption, /Open-source contributors/)
    assert.match(adoption, /## Repo Conversion Review/)
    assert.match(adoption, /README first viewport/)
    assert.match(adoption, /Social preview clarity/)
    assert.match(adoption, /Repo About and topics/)
    assert.match(adoption, /Discussions/)
    assert.match(adoption, /Release notes/)
    assert.match(adoption, /## 30-Day Checklist/)
    assert.match(adoption, /Days 1-3/)
    assert.match(adoption, /Days 28-30/)
    assert.match(adoption, /## Measurement/)
    assert.match(adoption, /GitHub stars and forks/)
    assert.match(adoption, /npm downloads/)
    assert.match(adoption, /README badge usage/)
    assert.match(adoption, /Issues and pull requests/)
    assert.match(adoption, /External mentions/)
  })
