...
/**
 * Read and parse .mdvprc / mdvp.config.json from cwd.
 * Returns the raw config object, or {} if none found / malformed.
 */
...
export function loadThresholds(cwd = process.cwd()) {
  const raw = loadRcConfig(cwd)
  const cfg = raw.thresholds ?? raw
  return { ...DEFAULT_THRESHOLDS, ...cfg }
}

/**
 * Load per-project signal config from .mdvprc.
 *   { signals: { disabled: string[], penalties: Record<string,number> } }
 * Returns { disabled, penalties } — empty defaults when absent.
 */
export function loadSignalConfig(cwd = process.cwd()) {
  const raw = loadRcConfig(cwd)
  const s = raw.signals ?? {}
  return { disabled: s.disabled ?? [], penalties: s.penalties ?? {} }
}
