...
function scoreVibecodedPenalty(metrics) {
  let score = 100
  const details = []
  const topFont = (metrics.fontFamilies || [])[0]?.[0] || ''
  const isSystemFont = ['-apple-system', 'system-ui', 'ui-sans-serif', 'Segoe UI', 'Helvetica Neue', 'Arial'].some(f => topFont.includes(f))
  const utility = isUtilitySite(metrics)
  if (isSystemFont && (metrics.fontFamilies || []).length <= 2 && !utility) { score -= 10; details.push(`System font stack only (${topFont}). No custom typeface`) }

  const vibecodeColors = ['rgb(59, 130, 246)', 'rgb(37, 99, 235)', 'rgb(96, 165, 250)', 'rgb(99, 102, 241)', 'rgb(79, 70, 229)', 'rgb(139, 92, 246)', 'rgb(168, 85, 247)', 'rgb(147, 51, 234)', 'rgb(236, 72, 153)', 'rgb(219, 39, 119)', 'rgb(244, 114, 182)', 'rgb(137, 110, 247)']
  const matchedVibeColors = (metrics.colors || []).filter(([c]) => vibecodeColors.some(vc => c.replace(/\s/g, '').includes(vc.replace(/\s/g, ''))))
  if (matchedVibeColors.length >= 2) { score -= 15; details.push(`Purple-pink-blue gradient palette (${matchedVibeColors.length} Tailwind accents). Hallmark of AI-generated design`) }
  else if (matchedVibeColors.length === 1) { score -= 5; details.push('Default Tailwind accent color detected') }

  const vibecodeFonts = ['Inter', 'Poppins', 'Nunito', 'Nunito Sans', 'Outfit']
  const isVibecodeFont = vibecodeFonts.some(f => topFont.includes(f))
  if (isVibecodeFont) { score -= 15; details.push(`${topFont} — default AI vibe-coding font. No typographic personality`) }

  const sizes = (metrics.fontSizes || []).map(([v]) => parsePx(v)).filter(v => !isNaN(v))
  const maxSize = sizes.length > 0 ? Math.max(...sizes) : 0
  const minSize = sizes.length > 0 ? Math.min(...sizes) : 0
  if (maxSize >= 48 && metrics.totalElements < 100) { score -= 10; details.push(`Oversized hero text (${maxSize}px) with sparse content`) }
  if (minSize > 0 && maxSize / minSize > 5) { score -= 5; details.push(`Extreme type scale ratio: ${(maxSize/minSize).toFixed(1)}x`) }

  const uniquePaddingValues = (metrics.paddings || []).map(([v]) => parsePx(v)).filter(v => !isNaN(v))
  if (uniquePaddingValues.some(v => v >= 100) && metrics.totalElements < 120) { score -= 5; details.push('Excessive padding (100px+) with low element count') }

  const tailwindSpacingScale = [4, 8, 12, 16, 20, 24, 32, 40, 48, 64, 80, 96, 128]
  const paddingPxValues = (metrics.paddings || []).map(([v]) => parsePx(v)).filter(v => !isNaN(v) && v > 0)
  const tailwindPaddingMatches = paddingPxValues.filter(v => tailwindSpacingScale.includes(v))
  if (paddingPxValues.length >= 4 && tailwindPaddingMatches.length / paddingPxValues.length > 0.85 && metrics.totalElements < 200) { score -= 15; details.push(`Tailwind default spacing scale detected (${Math.round(tailwindPaddingMatches.length / paddingPxValues.length * 100)}% match)`) }

  const pillEntry = (metrics.borderRadii || []).find(([v]) => v === '9999px' || v === '999px')
  const pillCount = pillEntry ? pillEntry[1] : 0
  if (pillCount > 8) { score -= 15; details.push(`${pillCount} pill-shaped elements (border-radius: 9999px). Hallmark of AI-generated design`) }
  else if (pillCount > 3) { score -= 5; details.push(`${pillCount} pill-shaped elements`) }
  const hasLargeRadius = (metrics.borderRadii || []).some(([v]) => parsePx(v) >= 16)
  if (hasLargeRadius && pillCount > 0) { score -= 5; details.push('Mixing large radius with pills') }

  if (metrics.totalElements < 60) { score -= 25; details.push(`Very sparse page (${metrics.totalElements} elements)`) }
  else if (metrics.totalElements < 100) { score -= 15; details.push(`Sparse page (${metrics.totalElements} elements)`) }
  else if (metrics.totalElements < 150 && (metrics.fontFamilies || []).length <= 2) { score -= 10; details.push(`Sparse page with minimal font variety`) }

  const onlyGray = (metrics.colors || []).every(([c]) => {
    const m = c.match(/(\d+)/g)
    if (!m || m.length < 3) return true
    const [r, g, b] = m.map(Number)
    return Math.abs(r - g) < 20 && Math.abs(g - b) < 20
  })
  if (onlyGray && (metrics.colors || []).length <= 5) { score -= 5; details.push('Monochrome palette without accent color') }

  let vibeSignals = 0
  if (metrics.totalElements < 200) vibeSignals++
  if (metrics.totalElements < 150) vibeSignals++
  if (maxSize > 90) vibeSignals++
  if (metrics.gradientCount && metrics.gradientCount > 5) vibeSignals++
  if (metrics.backdropBlurCount && metrics.backdropBlurCount > 2) vibeSignals++
  if (pillCount > 5) vibeSignals++
  if (metrics.divRatio && metrics.divRatio > 0.6) vibeSignals++
  if ((metrics.fontWeights || []).length > 4) vibeSignals++
  if (isSystemFont && (metrics.fontFamilies || []).length <= 2) vibeSignals++
  if (isVibecodeFont) vibeSignals += 2
  if ((metrics.fontFamilies || []).length <= 2) vibeSignals++
  if ((metrics.fontSizes || []).length <= 5) vibeSignals++
  if ((metrics.shadows || []).length <= 2 && metrics.totalElements < 200) vibeSignals++

  if (vibeSignals >= 8) { score -= 55; details.push(`Vibecoded composite: ${vibeSignals}/13 signals. Extremely strong AI-generated pattern`) }
  else if (vibeSignals >= 6) { score -= 40; details.push(`Vibecoded composite: ${vibeSignals}/13 signals. Strong AI-generated pattern`) }
  else if (vibeSignals >= 4) { score -= 25; details.push(`Vibecoded indicators: ${vibeSignals}/13 signals`) }
  else if (vibeSignals >= 2) { score -= 10; details.push(`Vibecoded indicators: ${vibeSignals}/13 signals`) }

  return { category: 'originality', score: Math.max(0, score), weight: 35, details }
}
...
export function scoreDOMMetrics(metrics) {
  ...
  else if (overall >= 76) grade = 'A-'
  else if (overall >= 70) grade = 'B+'
  else if (overall >= 64) grade = 'B'
  else if (overall >= 58) grade = 'B-'
  else if (overall >= 52) grade = 'C+'
  else if (overall >= 46) grade = 'C'
